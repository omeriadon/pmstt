...
struct AuthController: RouteCollection {
    ...
    func register(req: Request) async throws -> TokenResponse {
        ...
        let user = User(email: body.email, passwordHash: try req.password.hash(body.password), appleSubject: nil, displayName: body.displayName ?? "User", selfPassSerialNumber: UUID().uuidString, settingsData: try JSONEncoder().encode(AccountSettings.default))
    ...
    func signInWithApple(req: Request) async throws -> TokenResponse {
        ...
        let user = User(email: email, passwordHash: nil, appleSubject: appleSubject, displayName: resolvedDisplayName(body.displayName, fallbackEmail: email), selfPassSerialNumber: UUID().uuidString, settingsData: try JSONEncoder().encode(AccountSettings.default))
        try await user.save(on: req.db)
        return try await issueNewSession(for: user, platform: platform, installationID: normalizedInstallationID(body.installationID), on: req)
    }
    ...
    func refresh(req: Request) async throws -> TokenResponse {
        ...
        if session.platformValue == .watchOS { try await requireActiveParent(session, on: req) }
        ...
        let rotated = try await req.db.transaction { database -> UserToken in
            let query = UserToken.query(on: database)
                .filter(\.$id == (try session.requireID()))
            ...
            guard let rotated = try await UserToken.find(session.requireID(), on: database),
                  rotated.tokenHash == hashToken(refresh), rotated.refreshJTI == newJTI else {
    ...
    private func issueNewSession(for user: User, platform: ClientPlatform, installationID: String, parentSessionID: UUID? = nil, on req: Request) async throws -> TokenResponse {
        let session = UserToken(id: UUID(), tokenHash: "pending", userID: try user.requireID(), expiresAt: Date().addingTimeInterval(refreshLifetime(for: platform)), clientPlatform: platform.rawValue, installationID: installationID, parentSessionID: parentSessionID, refreshJTI: UUID())
        let refresh = try await makeRefreshToken(for: session, jti: session.refreshJTI!, on: req)
        session.tokenHash = hashToken(refresh)
    ...
    private func response(for session: UserToken, on req: Request, refreshToken: String? = nil) async throws -> TokenResponse {
        ...
        let access = try await req.jwt.sign(UserPayload(sub: try user.requireID(), sid: try session.requireID(), platform: session.platformValue, installationID: session.installationID ?? "", capabilities: session.platformValue.capabilities, expiresAt: Date().addingTimeInterval(60 * 15)))
		let refresh: String
		if let refreshToken {
			refresh = refreshToken
		} else {
			refresh = try await makeRefreshToken(for: session, jti: session.refreshJTI!, on: req)
		}
        return TokenResponse(accessToken: access, refreshToken: refresh, user: UserAccountResponse(id: try user.requireID(), email: user.email, displayName: user.displayName, createdAt: user.createdAt))
    }

    private func makeRefreshToken(for session: UserToken, jti: UUID, on req: Request) async throws -> String {
		try await req.jwt.sign(RefreshPayload(sub: session.$user.id, sid: try session.requireID(), platform: session.platformValue.rawValue, installationID: session.installationID ?? "", authority: session.platformValue.authority.rawValue, jti: jti, typ: "refresh", iss: .init(value: "pmstt"), iat: .init(value: Date()), exp: .init(value: Date().addingTimeInterval(refreshLifetime(for: session.platformValue)))))
    }

    private func newRefreshToken(for session: UserToken, jti: UUID?, legacy: Bool, on req: Request) async throws -> String {
        if legacy { return Data((0 ..< 32).map { _ in UInt8.random(in: 0 ... 255) }).base64EncodedString() }
    ...
    private func refreshLifetime(for platform: ClientPlatform) -> TimeInterval { platform == .watchOS ? 60 * 60 * 24 * 30 : 60 * 60 * 24 * 90 }
    private func validatedClient(platform raw: String, installationID: String) throws -> ClientPlatform { guard let platform = ClientPlatform(rawValue: raw), platform.signupAllowed else { throw AppError(.forbidden, code: .invalidRequest, reason: "This client platform cannot create a session.", field: "platform") }; _ = try normalizedInstallation(installationID); return platform }
    private func normalizedInstallationID(_ value: String) -> String { value.trimmingCharacters(in: .whitespacesAndNewlines) }
    private func normalizedInstallation(_ value: String) throws -> String { let result = normalizedInstallationID(value); guard !result.isEmpty, result.count <= 200 else { throw AppError(.badRequest, code: .invalidRequest, reason: "The installation identifier is invalid.", field: "installationID") }; return result }
    private func hashToken(_ token: String) -> String { SHA256.hash(data: Data(token.utf8)).map { String(format: "%02x", $0) }.joined() }
    private func resolvedDisplayName(_ value: String?, fallbackEmail: String?) -> String { let trimmed = value?.trimmingCharacters(in: .whitespacesAndNewlines) ?? ""; if !trimmed.isEmpty { return trimmed }; if let email = fallbackEmail, let prefix = email.split(separator: "@").first { return String(prefix) }; return "User" }
}

extension RegisterRequest: Validatable {
    static func validations(_ validations: inout Validations) { validations.add("email", as: String.self, is: .email); validations.add("password", as: String.self, is: .count(8...)) }
}
