...
enum AuthoritativeTimetableSource {
	...
	func subjects() throws -> [TimetableSubjectDTO] {
		guard subjectsData.count <= 1_048_576 else {
			throw AppError(
				.internalServerError,
				code: .internalServerError,
				reason: "Stored timetable data is too large."
			)
		}
		...
		do {
			...
			guard values.count <= 250 else {
				throw DecodingError.dataCorrupted(
					.init(codingPath: [], debugDescription: "Too many subjects.")
				)
			}
			return values
	...
	func preview() throws -> SharedTimetablePreview {
		...
		return try SharedTimetablePreview(
			id: id,
			title: title,
			authorAccountID: author.requireID(),
			authorDisplayName: author.displayName,
			sourceKind: sourceKind,
			revision: revision,
			updatedAt: updatedAt,
			subjectCount: values.count,
			weeklyLessonCount: values.reduce(0) { $0 + $1.slots.count },
			isImportable: isSearchable
		)
	}
...
enum AuthoritativeTimetableResolver {
	...
	static func resolveMany(
		ids: Set<UUID>,
		on database: any Database
	) async throws -> [SourceKey: AuthoritativeTimetableSource] {
		guard !ids.isEmpty else {
			return [:]
		}

		let owners = try await OwnerTimetable.query(on: database)
			.filter(\.$id ~~ ids)
			.with(\.$user)
			.all()
	...
	static func resolve(
		id: UUID,
		on database: any Database
	) async throws -> AuthoritativeTimetableResolution {
		guard let owner = try await OwnerTimetable.query(on: database)
			.filter(\.$id == id)
			.with(\.$user)
			.first()
		else {
		...
		if owner.isSearchable {
			return .available(.owner(owner))
		}
		return .privateSource(.owner(owner))
	}

	static func resolvePublic(
		id: UUID,
		on database: any Database
	) async throws -> AuthoritativeTimetableSource {
		guard case let .available(source) = try await resolve(id: id, on: database) else {
	...
	static func resolveForImport(
		...
		switch try await resolve(id: id, on: database) {
			case let .available(source):
				guard try source.author.requireID() != userID else {
					throw Abort(.notFound)
				}
				return source
			case .ambiguous:
				throw Abort(.conflict)
			case .privateSource, .missing:
				throw Abort(.notFound)
		}
	}
	...
	static func resolveForViewer(
		...
		switch try await resolve(id: id, on: database) {
			...
			case let .privateSource(source):
				...
				let isImported = try await hasImport(
					userID: userID,
					id: id,
					kind: source.sourceKind,
					on: database
				)
				return isAuthor || isImported ? .available(source) : .privateSource(source)
			case .missing, .ambiguous:
				return try await hasImport(userID: userID, id: id, kind: nil, on: database)
					? .missing
					: resolve(id: id, on: database)
		}
	}
	...
	private static func resolveAlias(
		locator: String,
		on database: any Database
	) async throws -> TimetableShareAlias {
		let canonical: String
		do {
			canonical = try TimetableShareAliasValidator.validateAndCanonicalize(locator)
		} catch {
			throw Abort(.notFound)
		}

		guard let alias = try await TimetableShareAlias.query(on: database)
	...
	static func hasImport(
		userID: UUID,
		id: UUID,
		kind: SourceKind?,
		on database: any Database
	) async throws -> Bool {
		var query = ReceivedTimetableImport.query(on: database)
			.filter(\.$user.$id == userID)
			.filter(\.$timetableID == id)
