...
enum PassFactory {
	...
	static func deletedResponse(record: PassRecord, req: Request) async throws -> Response {
		...
		return try await response(serial: record.serialNumber, displayName: "Deleted Timetable", subjectsData: empty, token: authenticationToken(serial: record.serialNumber), isDeleted: true, issuerAccountID: record.issuerAccountID, sourceKind: record.sourceKind, authorDisplayName: nil, isShareable: false, contentRevision: record.revision, req: req)
	}
	...
	private static func response(serial: String, displayName: String, subjectsData: Data, token: String, isDeleted: Bool, issuerAccountID: String, sourceKind: SourceKind, authorDisplayName: String?, isShareable: Bool, contentRevision: Int, req: Request) async throws -> Response {
		...
		let fileURL = try await generatePass(serialNumber: serial, displayName: displayName, subjectsData: subjectsData, authenticationToken: token, webServiceURL: Environment.get("PASS_WEB_SERVICE_URL") ?? "https://timetable.adonis.pt/v1", isDeleted: isDeleted, issuerAccountID: issuerAccountID, sourceKind: sourceKind, authorDisplayName: authorDisplayName, isShareable: isShareable, contentRevision: contentRevision, resourceDirectory: resources)
